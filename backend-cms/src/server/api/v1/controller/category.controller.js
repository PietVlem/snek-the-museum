...
class CategoryController {
    ...
    update = async (req, res, next) => {
        ...
        try {
            ...
            const category = await Category.findOneAndUpdate({ _id: id }, categoryUpdate, { new: true }).exec();
    ...
    destroy = async (req, res, next) => {
        ...
        try {
            ...
            if (!category) {
                ...
                return res.status(200).json({ message: `Successful deleted the Category with id: ${id}!`, category, mode });
            }
        } catch (err) {
            return handleAPIError(err.status || 500, err.message || `Some error occurred while deleting the Category with id: ${id}!`, next);
        }
