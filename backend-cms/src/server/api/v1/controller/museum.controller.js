...
class MuseumController {
    ...
    show = async (req, res, next) => {
        try {
            const { id } = req.params;
            const item = await Museum.findById(id).populate('category').populate('zipcode').populate('photo').populate('disability').populate('museumGallery').populate('exhibitions').exec();
